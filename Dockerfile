...
# Install mysql dev for R packages
RUN ["/bin/bash", "-c", "sudo apt-get install libmysqlclient-dev -y"]
...
RUN ["/bin/bash", "-c", "echo \"install.packages(c('xts','scatterplot3d','Metrics','caret','kernlab','nnet','quantmod'), repos='http://cran.univ-paris1.fr/',lib='/usr/local/lib/R/site-library', dependencies=TRUE)\" | sudo R --no-save"]
